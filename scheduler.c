        ...
        while (1)
        {
           ...
           // we need to check wheter we have enough memory or not 
            switch(memoryAlgorithm){
                ...
                flag=FirstFit_allocateNewProcess(&recProcess.process);
                break;
                case BUDDYMEMORY:
            ...
            if(flag){
            printf("Process %d allocated memory start %d end %d\n",processTable[recProcess.process.processNumber].memStart,processTable[recProcess.process.processNumber].memStart+recProcess.process.memSize-1);
            ...
            // printf("%d\n",curSize);
                ...
                Queue_push(&memoryWaitiingList,&recProcess.process);
            }
            processNumbers--;
        }
...
short FirstFit_allocateNewProcess(process_par* process){
    short flag=false;
    int processSize=process->memSize;
    int i,j;
    for( i=0, j = 0;i<TOTAL_MEMORY_SIZE;i=j+1){
        ...
        while(j < TOTAL_MEMORY_SIZE && memory[j] == -1) j++;
        int len = j-i;
        if (len < process->memSize) continue;
        for (int it = i; it < i+processSize; it++) memory[it] = 1;
        flag=true;
        break;
    }
    if(flag)
    processTable[process->processNumber].memStart=i;
    return flag;

}
short FirstFit_deAllocateProcess(process_par* process){
    int start=processTable[process->processNumber].memStart;
    for(int i=start;i<i+process->memSize;i++){
        memory[i]=-1;
    }
    return true;
