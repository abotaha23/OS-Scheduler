...
void Scheduler_recieveNewProcess(void *container)
{
    ...
    if (Scheduler == HPF || Scheduler == SRTN)
    {
        ...
        while (1)
        {
            ...
            // printf("nowTime %d Recieved process %d arrival time is %d\n", getClk(), recProcess.process.processNumber, recProcess.process.arrival_time);
...
short FirstFit_deAllocateProcess(int processNumber)
{
    ...
    printf("process %d will deallocate its memory\n",processNumber);
